...
def solve_optimal_control(x0, lmbd0, t_span, alpha, beta):
    ...
    sol_master = solve_bvp(lambda t, y: total_derivative_vec(t, y, alpha, \
                           ...
                           tmsh.shape[0])), verbose=2, tol=1e-6, \
                                                max_nodes = 5e4)
...
total_cost_cis = R()[0][0] * np.trapz(np.array(sol_u[0][0])**2, t1)
total_cost_pac = R()[1][1] * np.trapz(np.array(sol_u[0][1])**2, t1)
